...
void PrintObject::apply_extruder_layer_heights()
{
    ...
    auto extruder_idx_of = [&print_config, mapped_extruders](int filament_id) {
        const unsigned int filament = (unsigned int)std::max(1, filament_id) - 1;
        return mapped_extruders ? get_extruder_index(print_config, filament) : size_t(filament);
    };
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++ region_id) {
        ...
        const double nozzle_diameter = print_config.nozzle_diameter.get_at(
            extruder_idx_of(this->printing_region(region_id).config().outer_wall_filament_id.value));
        ...
        double min_layer_height = 0.;
        {
            ...
            if (walls_only) {
                ...
                pitch_filaments.emplace_back((unsigned int)std::max(1, region_config.outer_wall_filament_id.value) - 1);
                if (region_prints_inner_walls(region_config))
                    pitch_filaments.emplace_back((unsigned int)std::max(1, region_config.inner_wall_filament_id.value) - 1);
            } else {
            ...
            for (unsigned int filament : pitch_filaments)
                min_layer_height = std::max(min_layer_height, print_config.min_layer_height.get_at(
                    extruder_idx_of(int(filament) + 1)));
        }
        size_t min_run = 1;
