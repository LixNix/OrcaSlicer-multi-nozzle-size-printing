...
public:
    ...
    // explicit layer height preference - its top/bottom/solid feature filaments. Always returns true on this fork
    // (the Snapmaker fork returns false for mixed virtual filaments, which forbid combining).
    ...
    // Always returns true on this fork (the Snapmaker fork returns false for mixed virtual wall
    // filaments, which forbid wall combining).
...
public:
    ...
    */
    ...
    // Per-extruder vector index of a 0-based filament: classic multi-tool printers index by the
    // filament directly; 2-extruder BBL-style printers reroute through the filament map.
    size_t extruder_index_of(unsigned int filament_idx) const {
        return m_config.nozzle_diameter.size() == 2 && this->is_BBL_printer() ? get_extruder_index(m_config, filament_idx) : size_t(filament_idx);
    }
...
private:
    ...
    // uninitialized by CLI/tests - BBL-only code paths (filament grouping, wipe tower type)
    // key off it.
