    ...
    if (this->extruder_override == 0) {
        if (extrusions.has_infill()) {
            if (extrusions.has_solid_infill()) {
                ...
                else if (role == erBottomSurface || role == erBridgeInfill) // ORCA: external bridges print as bottom surfaces (internal bridges stay internal solid)
        ...
        for (const LayerRegion *layerm : layer->regions()) {
            ...
            if (! layerm->perimeters.entities.empty()) {
                ...
                if (something_nonoverriddable){
                    ...
                    // alternate_extra_wall should add an inner loop on odd layers (mirrors PerimeterGenerator'sloop_number and the spiral gate of LayerRegion::make_perimeters()); layers dropping the loop again (only_one_wall_top / only_one_wall_first_layer) may reserve the filament unused, which merely costs a toolchange.
            ...
            if (something_nonoverriddable || !m_print_config_ptr) {
                ...
                } else if (has_internal_solid || has_top_solid_surface || has_bottom_surface || has_infill || has_pure_gap_fill)
            		layer_tools.extruders.emplace_back(extruder_override);
            }
            if (has_internal_solid || has_top_solid_surface || has_bottom_surface || has_infill || has_pure_gap_fill)
                layer_tools.has_object = true;
        }
        layerCount++;
    }
...
static double filament_nozzle_diameter(const Print &print, unsigned int filament_id)
{
    ...
    const size_t extruder_idx = config.nozzle_diameter.size() == 2 && print.is_BBL_printer() ?
        get_extruder_index(config, filament_id) : size_t(filament_id);
    return config.nozzle_diameter.get_at(extruder_idx);
}
