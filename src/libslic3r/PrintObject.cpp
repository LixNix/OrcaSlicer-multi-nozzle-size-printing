...
static void apply_to_print_region_config(PrintRegionConfig &out, const DynamicPrintConfig &in, FeatureFilamentOverrideMask &feature_overrides, std::vector<int>& variant_index, int &base_extruder_through_scopes)
{
...
PrintRegionConfig region_config_from_model_volume(const PrintRegionConfig &default_or_parent_region_config, const DynamicPrintConfig *layer_range_config, const ModelVolume &volume, size_t num_extruders, std::vector<int>& variant_index)
{
    ...
    if (volume.is_model_part()) {
        ...
        apply_to_print_region_config(config, volume.get_object()->config.get(), feature_overrides, variant_index, base_extruder_through_scopes);
    } else {
        // default_or_parent_region_config contains parent PrintRegion config, which already contains ModelVolume's config.
    }
    apply_to_print_region_config(config, volume.config.get(), feature_overrides, variant_index, base_extruder_through_scopes);
    if (! volume.material_id().empty())
        apply_to_print_region_config(config, volume.material()->config.get(), feature_overrides, variant_index, base_extruder_through_scopes);
    if (layer_range_config != nullptr) {
        // Not applicable to modifiers.
        assert(volume.is_model_part());
    	apply_to_print_region_config(config, *layer_range_config, feature_overrides, variant_index, base_extruder_through_scopes);
    }
...
// caller which case applies. The bool return mirrors the Snapmaker fork (see Print.hpp).
...
// multipliers stay unadjusted and the walls merge at the lower height as usual. The bool
// return mirrors the Snapmaker fork (see Print.hpp).
...
void PrintObject::combine_infill()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++ region_id) {
        ...
        auto filament_nozzle = [this](int filament_id) {
            return this->print()->config().nozzle_diameter.get_at(this->print()->extruder_index_of(feature_filament_idx(filament_id)));
        };
