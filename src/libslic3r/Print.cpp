        ...
        for (PrintObject *object : m_objects) {
            ...
            if (layer_height > EPSILON) {
                ...
                for (const PrintRegion &region : object->all_regions()) {
                    ...
                    object->collect_region_pitch_filaments(region_config, lattice_filaments, pitch_from_features);
                    // Physical extruder of a 0-based filament (2-extruder BBL-style printers reroute
                    // filaments through the filament map).
                    auto extruder_of = [&](unsigned int filament) {
                        return mapped_extruders ? get_extruder_index(m_config, filament) : size_t(filament);
                    };
                    ...
                    if (region_config.wall_loops.value > 0) {
                        auto merge_wall = [&](int filament_id) {
                        ...
                        merge_wall(region_config.outer_wall_filament_id.value);
                        if (PrintObject::region_prints_inner_walls(region_config))
                            merge_wall(region_config.inner_wall_filament_id.value);
                        if (wall_candidate > 1 && ! wall_nozzle_fits(wall_candidate))
                            wall_candidate = 1;
                    }
                    if (wall_candidate == 0)
                        wall_candidate = 1;
